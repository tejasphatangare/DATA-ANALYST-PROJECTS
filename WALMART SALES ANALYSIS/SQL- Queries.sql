...
select Sub_Category,round(sum(Total),2) as 'Product Line With Revenue' from walmart
group by Sub_category
order by `Product Line With Revenue` desc;

-- 5-What product line had the largest VAT?
select Sub_category,round(max(Tax_5_Per),2)as 'Product Line With Max Tax' from walmart
group by Sub_category
...
select month_name,max(cogs)as 'Month With Max COGS' from walmart
group by month_name
...
select Branch,round(count(Quantity),2)as 'Branch With Most Product' from walmart
group by Branch
order by `Branch With Most Product` desc;
...
-- 1-Number of sales made by quantity in each time of the day per weekday.
select day_name,time_of_day,count(Quantity)as 'Sales Made In Quantity' from walmart
group by time_of_day,day_name
order by `Sales Made In Quantity` desc;

select time_of_day,count(Quantity)as 'Sales Made In Quantity' from walmart
...
select day_name,round(avg(Rating),1)as 'Avg_Rating' from walmart
group by day_name
